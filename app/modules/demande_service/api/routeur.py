...
from app.modules.demande_service.api.schemas import (
    RequestServiceCreateRequest,
    RequestServiceSchema,
    RequestServiceStatusUpdateRequest,
)
from app.modules.demande_service.application.cas_utilisation import (
...
from app.modules.demande_service.infrastructure.depots import (
    SQLAlchemyRequestServiceRepository,
)
from app.shared.schemas.pagination import Page, PaginationParams

router = APIRouter(tags=["Demandes de Service"])
...
async def create_request_service(
    ...
    req = await uc.execute(id_utilisateur=current_user["id"], **payload.model_dump())
    return RequestServiceSchema.model_validate(req)


@router.get("/demandes-service", response_model=Page[RequestServiceSchema])
async def list_my_request_services(
    ...
    return await uc.execute(id_utilisateur=current_user["id"], params=params)  # type: ignore[return-valeur]


@router.get("/demandes-service/{request_id}", response_model=RequestServiceSchema)
async def get_my_request_service(
    ...
    try:
        req = await uc.execute(request_id=request_id, id_utilisateur=current_user["id"])
        ...
        raise HTTPException(status_code=statut.HTTP_403_FORBIDDEN, detail=exc.message) from exc
    return RequestServiceSchema.model_validate(req)


@router.delete(
    "/demandes-service/{request_id}",
...
async def cancel_my_request_service(
    ...
    try:
        await uc.execute(request_id=request_id, id_utilisateur=current_user["id"])
    except RequestServiceNotFoundError as exc:
        raise HTTPException(status_code=statut.HTTP_404_NOT_FOUND, detail=exc.message) from exc
    except RequestServiceAccessDeniedError as exc:
        raise HTTPException(status_code=statut.HTTP_403_FORBIDDEN, detail=exc.message) from exc
...
async def admin_list_request_services(
    page: Annotated[int, Query(ge=1)] = 1,
    per_page: Annotated[int, Query(ge=1, le=100)] = 20,
    status_filter: Annotated[RequestServiceStatus | None, Query(alias="statut")] = None,
    repo: SQLAlchemyRequestServiceRepository = Depends(_repo),
) -> Page[RequestServiceSchema]:
    params = PaginationParams(page=page, per_page=per_page)
    uc = ListAllRequestServicesUseCase(repo)
    return await uc.execute(params=params, statut=status_filter)  # type: ignore[return-valeur]


@router.patch(
    "/admin/demandes-service/{request_id}/statut",
    response_model=RequestServiceSchema,
    dependencies=[Depends(require_role("admin", "super-admin"))],
)
async def admin_update_request_service_status(
    ...
    try:
        req = await uc.execute(request_id=request_id, new_status=payload.statut)
    except RequestServiceNotFoundError as exc:
        raise HTTPException(status_code=statut.HTTP_404_NOT_FOUND, detail=exc.message) from exc
    return RequestServiceSchema.model_validate(req)
