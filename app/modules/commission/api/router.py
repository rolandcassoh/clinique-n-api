"""Router FastAPI du module commission."""
...
from app.modules.commission.application.use_cases import (
...
from app.modules.commission.infrastructure.repositories import (
...
@router.get("/doctor/earnings", response_model=Page[CommissionEarningSchema])
...
@router.get("/doctor/earnings/summary", response_model=DoctorEarningsSummarySchema)
...
async def admin_list_commissions(
    ...
    clinic_id: Annotated[Optional[int], Query()] = None,
    doctor_id: Annotated[Optional[int], Query()] = None,
    filter_status: Annotated[Optional[str], Query(alias="status")] = None,
    page: Annotated[int, Query(ge=1)] = 1,
    per_page: Annotated[int, Query(ge=1, le=100)] = 20,
    earning_repo: SQLAlchemyCommissionEarningRepository = Depends(_earning_repo),
) -> Page[CommissionEarningSchema]:
    ...
    return await uc.execute(params, clinic_id, doctor_id, filter_status)  # type: ignore[return-value]


@router.post(
    "/admin/commissions/{commission_id}/mark-paid",
...
@router.get("/admin/earnings", response_model=Page[EmployeeEarningSchema])
